...
| `frontend-app-structure-with-example-pages.md` | Prompt para generar la estructura de la app frontend (carpetas api, hooks, pages, queries, router, store, styles, tests), archivos de entrada (main, app, router, store, api client, styles, test-utils) y dos páginas de ejemplo (Home e ítem por id).                      |
| `backend-app-structure-with-example-api.md`    | Prompt para generar la estructura de la app backend (carpetas controllers, services, routes, types, data, openapi, lib), archivos de entrada (main, app), lib (logger, request-logger) y una API básica de ejemplo (recurso items: GET list y GET por id) con OpenAPI.       |
| `home-carousel-redesign-spec.md`               | Prompt para reproducir la spec de mejora del home: carousel fullscreen con autoplay para el banner publicitario y carousel de tarjetas flotante encima del banner, alineado con el diseño de Mercado Libre Argentina.                                                        |
